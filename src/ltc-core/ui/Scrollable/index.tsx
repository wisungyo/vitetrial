...
const SfScrollable = polymorphicForwardRef<
  typeof defaultScrollableTag,
  SfScrollableProps
>(
  (
    {
    ...
    const { state, containerRef, getNextButtonProps, getPrevButtonProps } =
      useScrollable(sliderOptions);

    function PreviousButton({ classNameButton }: { classNameButton?: string }) {
      if (slotPreviousButton) {
        return cloneElement(
          slotPreviousButton,
          getPrevButtonProps({ disabled: prevDisabled, onClick: onPrev })
        );
      }
      return (
        <SfButton
          {...getPrevButtonProps({
            ...
            className: classNames(
              ...
              isFloating
                ? "disabled:hidden"
                : "disabled:!ring-disabled-300 disabled:!text-disabled-500"
            ),
          })}
        />
      );
    ...
    function NextButton({ classNameButton }: { classNameButton?: string }) {
      if (slotNextButton) {
        return cloneElement(
          slotNextButton,
          getNextButtonProps({ disabled: nextDisabled, onClick: onNext })
        );
      }
      return (
        <SfButton
